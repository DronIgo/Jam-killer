...
public class Health : MonoBehaviour
{
    ...
    public Transform movableObjects = null;
    /// <summary>
    /// Description:
    ...
    void Start()
    {
        ...
        currentLives = maximumLives;//костыль
    }
    ...
    void Respawn()
    {
        ...
        if (movableObjects != null)
            foreach (Transform child in movableObjects)
            {
                child.gameObject.GetComponent<MovableObject>().ResetPosition();
            }
        if (TryGetComponent(out PlayerController playerController))
        {
            for (int i = 0; i < playerController.backpack.Count; i++) {
                playerController.backpack[i].GetComponent<MovableObject>().ResetPosition();
                playerController.backpack.RemoveAt(i);
                playerController.canBePlaced.RemoveAt(i);
            }
        }
    }
